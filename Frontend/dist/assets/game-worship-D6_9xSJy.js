import{P as h}from"./phaser-iZDVk5aZ.js";const b=new URL("/assets/%E0%B9%84%E0%B8%AB%E0%B8%A7%E0%B9%89%E0%B8%9E%E0%B8%A3%E0%B8%B0%E0%B9%80%E0%B8%A3%E0%B8%B4%E0%B9%88%E0%B8%A1%E0%B9%80%E0%B8%81%E0%B8%A1-Bos3kp7C.png",import.meta.url).href,u=new URL("/assets/temple_bell-zYhLx0tT.wav",import.meta.url).href,w=new URL("/assets/calm_ambience-BqQWv_i4.wav",import.meta.url).href,l=[{id:"incense",label:"ธูป",icon:"🪔",color:"#ffcf8a"},{id:"candle",label:"เทียน",icon:"🕯️",color:"#ffe59e"},{id:"lotus",label:"ดอกบัว",icon:"🪷",color:"#ffcde0"}],c=["สุขภาพแข็งแรง","โชคดีตลอดปี","การงานราบรื่น","มีคนเมตตาอุปถัมภ์","สมหวังในสิ่งที่ตั้งใจ"],r=3,p=45;class f extends h.Scene{constructor(){super({key:"WorshipBoothScene"}),this.onGameEnd=null,this.root=null,this.state=null,this.timerHandle=null,this.calmBgm=null}init(t={}){this.onGameEnd=(t==null?void 0:t.onGameEnd)??null}preload(){this.load.audio("worship-bell",u),this.load.audio("worship-calm",w)}create(){this.state=this.createInitialState(),this.buildDom(),this.renderState(),this.events.once("shutdown",()=>this.cleanup()),this.events.once("destroy",()=>this.cleanup())}createInitialState(){return{phase:"intro",score:0,mistakes:0,round:1,timeLeft:p,stepIndex:0,roundSequence:this.generateSequence(),completedRounds:0,blessing:null}}generateSequence(){return h.Utils.Array.Shuffle([...l]).map(t=>t.id)}buildDom(){var e,s,o,i,d;const t=(e=this.game.canvas)==null?void 0:e.parentElement;t&&(this.root=document.createElement("div"),this.root.innerHTML=`
      <style>
        ...
        <div class="wb-wrap">
          <div class="wb-card">
            ...
            <div class="wb-hud">
              ...
              <div class="wb-box"><strong>รอบ</strong><span id="wb-round">1 / ${r}</span></div>
    ...
    `,t.appendChild(this.root),this.scoreEl=this.root.querySelector("#wb-score"),this.timeEl=this.root.querySelector("#wb-time"),this.roundEl=this.root.querySelector("#wb-round"),this.mistakesEl=this.root.querySelector("#wb-mistakes"),this.sequenceEl=this.root.querySelector("#wb-sequence"),this.msgEl=this.root.querySelector("#wb-msg"),this.gridEl=this.root.querySelector("#wb-grid"),this.prayBtn=this.root.querySelector("#wb-pray"),this.introEl=this.root.querySelector("#wb-intro"),this.countdownEl=this.root.querySelector("#wb-countdown"),this.countValueEl=this.root.querySelector("#wb-count"),this.resultEl=this.root.querySelector("#wb-result"),this.finalBlessingEl=this.root.querySelector("#wb-final-blessing"),this.finalScoreEl=this.root.querySelector("#wb-final-score"),this.finalMetaEl=this.root.querySelector("#wb-final-meta"),l.forEach(n=>{const a=document.createElement("button");a.className="wb-btn",a.dataset.stepId=n.id,a.innerHTML=`<span>${n.icon}</span>${n.label}`,a.addEventListener("click",()=>this.pickStep(n.id,a)),this.gridEl.appendChild(a)}),(s=this.root.querySelector("#wb-start-btn"))==null||s.addEventListener("click",()=>this.startCountdown()),(o=this.root.querySelector("#wb-help-btn"))==null||o.addEventListener("click",()=>{this.msgEl.textContent="กดให้ตรงตามลำดับที่แสดงด้านบน หากผิดจะต้องเริ่มลำดับรอบนั้นใหม่"}),(i=this.prayBtn)==null||i.addEventListener("click",()=>this.finishGame()),(d=this.root.querySelector("#wb-finish-btn"))==null||d.addEventListener("click",()=>{var n;(n=this.onGameEnd)==null||n.call(this,{score:this.state.score,blessing:this.state.blessing,meta:{mistakes:this.state.mistakes,completedRounds:this.state.completedRounds,timeLeft:this.state.timeLeft}})}))}startCountdown(){if(this.state.phase!=="intro")return;this.state.phase="countdown",this.introEl.style.display="none",this.countdownEl.style.display="flex";let t=3;this.countValueEl.textContent="3",this.countdownTimer=window.setInterval(()=>{var e;t-=1,t>0?this.countValueEl.textContent=String(t):t===0?this.countValueEl.textContent="เริ่ม!":(window.clearInterval(this.countdownTimer),this.countdownTimer=null,this.countdownEl.style.display="none",(e=this.cache.audio)!=null&&e.exists("worship-calm")&&(this.calmBgm=this.sound.add("worship-calm",{loop:!0,volume:.18}),this.calmBgm.isPlaying||this.calmBgm.play()),this.startRoundTimer(),this.state.phase="playing",this.msgEl.textContent=`เริ่มจาก ${this.stepLabel(this.state.roundSequence[0])}`,this.renderState())},1e3)}startRoundTimer(){this.timerHandle=window.setInterval(()=>{this.state.phase==="playing"&&(this.state.timeLeft-=1,this.state.timeLeft<=0&&(this.state.timeLeft=0,this.finishGame(!0)),this.renderState())},1e3)}stepLabel(t){var e;return((e=l.find(s=>s.id===t))==null?void 0:e.label)??t}pickStep(t,e){var i;if(this.state.phase!=="playing")return;const o=this.state.roundSequence[this.state.stepIndex]===t;if(e.classList.remove("wrong","right"),e.offsetWidth,e.classList.add(o?"right":"wrong"),window.setTimeout(()=>e.classList.remove("wrong","right"),240),!o){this.state.mistakes+=1,this.state.score=Math.max(0,this.state.score-5),this.state.stepIndex=0,this.msgEl.textContent=`ผิดลำดับ ต้องเริ่มรอบ ${this.state.round} ใหม่`,this.renderState();return}if(this.state.score+=15,this.state.stepIndex+=1,(i=this.cache.audio)!=null&&i.exists("worship-bell")&&this.sound.play("worship-bell",{volume:.18}),this.state.stepIndex>=this.state.roundSequence.length){if(this.state.completedRounds+=1,this.state.score+=20,this.state.round>=r){this.msgEl.textContent="พิธีครบแล้ว กดรับพรได้เลย",this.state.phase="ready",this.renderState();return}this.state.round+=1,this.state.stepIndex=0,this.state.roundSequence=this.generateSequence(),this.msgEl.textContent=`ผ่านรอบแล้ว เริ่มรอบ ${this.state.round} ด้วย ${this.stepLabel(this.state.roundSequence[0])}`,this.renderState();return}this.msgEl.textContent=`ต่อไป: ${this.stepLabel(this.state.roundSequence[this.state.stepIndex])}`,this.renderState()}finishGame(t=!1){var e,s;this.state.phase!=="result"&&(window.clearInterval(this.timerHandle),this.timerHandle=null,(e=this.calmBgm)==null||e.stop(),this.state.phase="result",this.state.blessing=c[Math.floor(Math.random()*c.length)],(s=this.cache.audio)!=null&&s.exists("worship-bell")&&this.sound.play("worship-bell",{volume:.35}),t||(this.state.score+=Math.max(10,this.state.timeLeft)),this.finalBlessingEl.textContent=this.state.blessing,this.finalScoreEl.textContent=String(this.state.score),this.finalMetaEl.textContent=t?`หมดเวลาก่อนจบพิธี พลาด ${this.state.mistakes} ครั้ง สำเร็จ ${this.state.completedRounds}/${r} รอบ`:`ทำพิธีครบ ${this.state.completedRounds}/${r} รอบ พลาด ${this.state.mistakes} ครั้ง เหลือเวลา ${this.state.timeLeft} วินาที`,this.resultEl.style.display="flex",this.renderState())}renderState(){this.scoreEl.textContent=String(this.state.score),this.timeEl.textContent=String(this.state.timeLeft),this.roundEl.textContent=`${Math.min(this.state.round,r)} / ${r}`,this.mistakesEl.textContent=String(this.state.mistakes),this.sequenceEl.innerHTML=this.state.roundSequence.map((t,e)=>{const s=l.find(i=>i.id===t);return`<div class="${["wb-pill",e<this.state.stepIndex?"done":"",e===this.state.stepIndex&&(this.state.phase==="playing"||this.state.phase==="ready")?"active":""].filter(Boolean).join(" ")}" style="background:${e<this.state.stepIndex?"#e7ffd8":s.color}">${s.icon} ${s.label}</div>`}).join(""),this.prayBtn.disabled=this.state.phase!=="ready"}cleanup(){var t,e,s;(t=this.calmBgm)==null||t.stop(),(e=this.calmBgm)==null||e.destroy(),this.calmBgm=null,window.clearInterval(this.timerHandle),window.clearInterval(this.countdownTimer),this.timerHandle=null,this.countdownTimer=null,(s=this.root)!=null&&s.parentNode&&this.root.parentNode.removeChild(this.root),this.root=null}}export{f as W};
